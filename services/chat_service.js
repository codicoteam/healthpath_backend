...
const saveMessage = async ({ groupId, senderId, message }) => {
  const newMessage = await Message.create({ groupId, sender: senderId, message });
  return newMessage;
};
...
const saveGroup = async (groupId) => {
  ...
  const saveUser = async (email, first_name, last_name, profile_picture, role, userId,online = false, lastSeen = null) => {
