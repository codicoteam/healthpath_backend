...
router.get('/visit/:visitId', authenticateToken, async (req, res) => {
    try {
        ...
        const medications = await medicationService.getMedicationsByVisitId(visitId);
        res.status(200).json({
...
router.get('/client/:clientId', authenticateToken, async (req, res) => {
    try {
        ...
        const medications = await medicationService.getMedicationsByClientid(clientId);
        res.status(200).json({
            message: 'Medications for visit retrieved successfully',
            data: medications,
        });
...
router.put('/update/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const updatedMedication = await medicationService.updateMedication(medicationId, updateData);
...
router.delete('/delete/:id', authenticateToken, async (req, res) => {
    try {
        const medicationId = req.params.id;
        const deletedMedication = await medicationService.deleteMedication(medicationId);
